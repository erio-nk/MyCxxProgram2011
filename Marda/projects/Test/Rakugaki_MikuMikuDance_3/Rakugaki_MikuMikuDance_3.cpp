// Rakugaki_MikuMikuDance_3.cpp : アプリケーションのエントリ ポイントを定義します。
//

#include "stdafx.h"
#include "Rakugaki_MikuMikuDance_3.h"
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_RAKUGAKI_MIKUMIKUDANCE_3, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_RAKUGAKI_MIKUMIKUDANCE_3));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_RAKUGAKI_MIKUMIKUDANCE_3));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_RAKUGAKI_MIKUMIKUDANCE_3);
...
public:
	...
	bool Create(Device& device)
	{
		...
		// シェーダの作成。
		const TCHAR* SHADER_FILE_PATH = _T("..\\Rakugaki_MikuMikuDance_3\\pmd.sh");
		...
		// 入力レイアウトの作成。
		{
			D3D10_INPUT_ELEMENT_DESC layout[] = {
				...
				{"BONE_INDICES", 0, DXGI_FORMAT_R32G32_UINT,     0, offsetof(ShaderInput, boneIndices), D3D10_INPUT_PER_VERTEX_DATA, 0},
				{"WEIGHT",       0, DXGI_FORMAT_R32_FLOAT,       0, offsetof(ShaderInput, weight), D3D10_INPUT_PER_VERTEX_DATA, 0},
			};
...
public:
	...
	bool Create(Device& device, const TCHAR* pmdPath, const TCHAR* vmdPath)
	{
		...
		{
			MMD::PMD::IKData& ik = pmd.ik_data[i];
			if (ik.ik_target_bone_index != 70) continue;
			_iks.push_back(IKInfo());
			IKInfo& ikInfo = _iks.back();
		#endif
