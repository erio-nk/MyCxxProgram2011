// Rakugaki_MikuMikuDance_6.cpp : アプリケーションのエントリ ポイントを定義します。
//

#include "stdafx.h"
#include "Rakugaki_MikuMikuDance_6.h"
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_RAKUGAKI_MIKUMIKUDANCE_6, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_RAKUGAKI_MIKUMIKUDANCE_6));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_RAKUGAKI_MIKUMIKUDANCE_6));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_RAKUGAKI_MIKUMIKUDANCE_6);
...
public:
	enum SHADER_KIND
	{
		PMD_TOON_EDGE_DEPTH,		//!< 深度マップを利用したエッジ描画。
		PMD_TOON_EDGE_NORMAL_DEPTH,		//!< 法線深度マップを利用したエッジ描画。
		PMD_MESH_MAX,

		PMD_BONE = PMD_MESH_MAX,	//!< ボーン描画用。
		PMD_EDGE_DEPTH,
		PMD_EDGE_NORMAL_DEPTH,
	...
	bool Create(Device& device)
	{
		...
		//// シェーダの作成。
		const TCHAR* SHADER_FILE_PATH = _T("..\\Rakugaki_MikuMikuDance_6\\pmd.sh");
		...
		// エッジ描画（深度マップ法）。
		ShaderDependencyObjects& shaderToonEdgeDepth = _shaderDependencyObjects[PMD_TOON_EDGE_DEPTH];
		shaderToonEdgeDepth._shader = device.CreateShader(
			SHADER_FILE_PATH,
			"VS", "vs_4_0", 
			NULL, NULL,
			"PS_Toon_Edge_Depth", "ps_4_0"
			);
		if (shaderToonEdgeDepth._shader == ShaderHandle::INVALID)
		{
			return false;
		}

		SetShaderTextureArray(shaderToonEdgeDepth._shader, "texToonArray", _texToons);

		ShaderDependencyObjects& shaderEdgeDepth = _shaderDependencyObjects[PMD_EDGE_DEPTH];
		shaderEdgeDepth._shader = device.CreateShader(
			SHADER_FILE_PATH,
			"VS", "vs_4_0", 
			NULL, NULL,
			"PS_Edge_Depth", "ps_4_0"
			);
		if (shaderEdgeDepth._shader == ShaderHandle::INVALID)
		{
			return false;
		}

		_depthMap = device.CreateDepthStencilBuffer(0, 0, true);
		if (_depthMap == DepthStencilBufferHandle::INVALID)
			return false;

		// エッジ描画（深度深度マップ法）。
		ShaderDependencyObjects& shaderToonEdgeNormalDepth = _shaderDependencyObjects[PMD_TOON_EDGE_NORMAL_DEPTH];
		shaderToonEdgeNormalDepth._shader = device.CreateShader(
			SHADER_FILE_PATH,
			"VS", "vs_4_0", 
			NULL, NULL,
			"PS_Toon_Edge_NormalDepth", "ps_4_0"
			);
		if (shaderToonEdgeDepth._shader == ShaderHandle::INVALID)
		{
			return false;
		...
		SetShaderTextureArray(shaderToonEdgeNormalDepth._shader, "texToonArray", _texToons);

		ShaderDependencyObjects& shaderEdgeNormalDepth = _shaderDependencyObjects[PMD_EDGE_NORMAL_DEPTH];
		shaderEdgeNormalDepth._shader = device.CreateShader(
			SHADER_FILE_PATH,
			"VS", "vs_4_0", 
			NULL, NULL,
			"PS_Edge_NormalDepth", "ps_4_0"
			);
		if (shaderEdgeNormalDepth._shader == ShaderHandle::INVALID)
		{
			return false;
		}

		_normalDepthMap = device.CreateRenderTarget(0, 0, DXGI_FORMAT_R32G32B32A32_FLOAT, true);
		if (_normalDepthMap == RenderTargetHandle::INVALID)
			return false;
		...
		// 入力レイアウトの作成。
		{
			D3D10_INPUT_ELEMENT_DESC layout[] = {
				...
				{"COLOR",        0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offsetof(ShaderInputVertex, diffuse), D3D10_INPUT_PER_VERTEX_DATA, 0},
				...
				{"EDGE_FLAG",    0, DXGI_FORMAT_R32_UINT,        0, offsetof(ShaderInputVertex, edgeFlag), D3D10_INPUT_PER_VERTEX_DATA, 0},
	...
	DepthStencilBufferHandle GetDepthMap() const
	{
		return _depthMap;
	}

	RenderTargetHandle GetNormalDepthMap() const
	{
...
private:
	...
	DepthStencilBufferHandle _depthMap;
	RenderTargetHandle _normalDepthMap;
...
public:
	...
	{
		SetShaderKind(MyRenderer::PMD_TOON_EDGE_NORMAL_DEPTH);
		...
		for (unsigned int i = 0, k = 0; i < pmd.material_count; ++i)
		{
			...
			for (; k < total; ++k)
			{
				...
				vertex.diffuse[3] = material.alpha;
				...
				vertex.edgeFlag = material.edge_flag;
			}
		...
		for (unsigned int i = 0; i < pmd.ik_data_count; ++i)
		{
			...
			for (size_t k = 0, ksz = ikInfo._chains.size(); k < ksz; ++k)
			{
				...
				if (boneChain._name.find("ひざ") != std::string::npos)
				{
					...
					// ろぽさんは -π<X<0 （≦ではない） and Y=Z=0 でやってます
				}
				else
...
//		rsDesc.MultisampleEnable = TRUE;
		...
		for (size_t i = 1; i < _countof(bsDesc.BlendEnable); ++i)
		{
...
//			// 加算合成 : src.rgb + dst.rgb
		...
		_blendState= device.CreateBlendState(bsDesc);
...
//		}
		...
		for (size_t i = 0; i < _drawPlans.size(); ++i)
		{
			...
			if (shaderKind != MyRenderer::PMD_EDGE_DEPTH && shaderKind != MyRenderer::PMD_EDGE_NORMAL_DEPTH)	// 本来なら、分岐しないでやれる方法が良い。
			{
			...
			renderer.SetBlendState(_blendState);
...
struct ApplicationFrame::Impl
{
	...
	enum FLAGS
	{
		...
		INVALID_EDGE_DRAW		= 0x08,
		INVALID_BONE_DRAW		= 0x10,
	};
	Flags _flags;
...
//				D3DXVec3TransformNormal(&_light._direction, &_light._direction, &m);
			...
			// モデル描画。
			{
				...
				{
					if (_pmd.GetShaderKind() == MyRenderer::PMD_TOON_EDGE_DEPTH)
					{
						// 深度マップに描画。
						_renderer.SetRenderTargets(_renderer.GetDefaultRenderTarget(), _renderer.GetDepthMap());
						_renderer.Clear(false, true, false, Color::BLUE);	// 深度バッファクリア。
						_pmd.DrawMesh(_renderer, MyRenderer::PMD_EDGE_DEPTH);
						
						// メッシュとエッジ描画。
						_renderer.SetDefaultRenderTargets();
						_renderer.Clear(true, true, false, Color::BLUE);
						_renderer.SetDepthStencilBufferToShaderTexture(_renderer.GetShader(_pmd.GetShaderKind()), "depthMap", _renderer.GetDepthMap());
						_pmd.DrawMesh(_renderer);
					}
					else
					if (_pmd.GetShaderKind() == MyRenderer::PMD_TOON_EDGE_NORMAL_DEPTH)
					{
						// 深度法線マップに描画。
						_renderer.SetRenderTargets(_renderer.GetNormalDepthMap(), _renderer.GetDefaultDepthStencilBuffer());
						_renderer.Clear(true, true, false, Color::BLACK);
						_pmd.DrawMesh(_renderer, MyRenderer::PMD_EDGE_NORMAL_DEPTH);
						
						// メッシュとエッジを描画。
						_renderer.SetDefaultRenderTargets();
						_renderer.Clear(true, true, false, Color::BLUE);
						_renderer.SetRenderTargetToShaderTexture(_renderer.GetShader(_pmd.GetShaderKind()), "normalDepthMap", _renderer.GetNormalDepthMap());
						_pmd.DrawMesh(_renderer);
					}
			...
			{
				case MyRenderer::PMD_TOON_EDGE_DEPTH: pShaderKind = _T("エッジ描画（深度マップ法）"); break;
				case MyRenderer::PMD_TOON_EDGE_NORMAL_DEPTH: pShaderKind = _T("エッジ描画（法線深度マップ法）"); break;
			}
